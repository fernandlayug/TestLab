...
selected_features_df.to_excel('selected_features_1.xlsx', index=False)
...
selected_data.to_excel('selected_data_1.xlsx', index=False)
...
plt.figure(figsize=(14, 10))
sns.heatmap(fold_scores_df, annot=True, cmap='coolwarm', center=0, fmt=".2f")
plt.xlabel('Features')
plt.ylabel('Folds')
plt.title('Heatmap of Feature Scores Across Folds')
plt.xticks(ticks=np.arange(len(X.columns)) + 0.5, labels=X.columns, rotation=90)
plt.show()
...
plt.xticks(ticks=np.arange(len(selected_features)), labels=selected_features, rotation=45)
plt.show()

# Visualization 4: Average scores of the selected features
plt.figure(figsize=(10, 6))
bars = plt.barh(selected_features, selector.scores_[np.argsort(selector.scores_)[-k_features:]], color='skyblue')
...
# Print the best alpha found by LassoCV
